...
$(document).ready(function() {
	...
	$(".a_title").click(function(e) {
		...
		// attr: attribute(속성)
		var cb_no = $(this).attr("data-bno"); // 489
		$("#frmPaging > input[name=cb_no]").val(b_no);
		$("#frmPaging").attr("action", "/customerboard/customerBoard");
		$("#frmPaging").submit();
	});
...
<form id="frmPaging" action="/customerboard/customerBoard" method="get">
	<input type="hidden" name="page" value="${pagingDto.page}"/>
	<input type="hidden" name="perPage" value="${pagingDto.perPage}"/>
	<input type="hidden" name="searchType" value="${pagingDto.searchType}"/>
	<input type="hidden" name="keyword" value="${pagingDto.keyword}"/>
	<input type="hidden" name="cb_no"/>
</form>
<!-- 배너 -->
                         ...
                         <div class="input-group-append">
                             ...
                             <c:choose>
								<c:when test="${empty sessionScope.loginVo }">
									...
									<c:otherwise>
									<a href="/customerboard/writeForm" class="btn btn-primary" id="customer">문의하기</a>
									</c:otherwise>
							</c:choose>
                         </div>
                     </div>
			...
			<div class="col-md-10">
				<h1>고객센터</h1>
					<table class="table">
						...
						<tbody>
						<c:forEach var="customerBoardVo" items="${list}">
							<tr>
								<td>${customerBoardVo.cb_no}</td>
								<td>
								<c:choose>
								<c:when test="${customerBoardVo.re_level > 0}">
									<a href="/customerboard/replyContent?cb_no=${customerBoardVo.cb_no}"
									 style="margin-left:${customerBoardVo.re_level * 30}px">
									 <c:if test="${customerBoardVo.re_level > 0}"> ㄴ </c:if>${customerBoardVo.cb_title}</a>
								</c:when>
									<c:otherwise>
										<a href="/customerboard/content?cb_no=${customerBoardVo.cb_no}"
										style="margin-left:${customerBoardVo.re_level * 30}px">
										<c:if test="${customerBoardVo.re_level > 0}"> ㄴ </c:if>${customerBoardVo.cb_title}</a>
									</c:otherwise>
								</c:choose>
								</td>
<%-- 								<td><a href="/customerboard/content?cb_no=${customerBoardVo.cb_no}" --%>
<%-- 								 style="margin-left:${customerBoardVo.re_level * 30}px"> --%>
<%-- 								 <c:if test="${customerBoardVo.re_level > 0}"> ㄴ </c:if>${customerBoardVo.cb_title}</a></td> --%>
								<td>${customerBoardVo.user_id}</td>
								<td>${customerBoardVo.cb_regdate}</td>
								<td>${customerBoardVo.cb_count}</td>
							</tr>
						</c:forEach>
						</tbody>
					</table>
